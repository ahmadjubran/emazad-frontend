...
export const getBlockedItem = (dispatch) => {
  axios
    .get(`${process.env.REACT_APP_HEROKU_API_KEY}/reportItems`)
    .then((res) => {
...
export const getUsersBlocked = (dispatch) => {
  axios
    .get(`${process.env.REACT_APP_HEROKU_API_KEY}/usersBlocked`)
    .then((res) => {
...
export const getNumberUsers = (dispatch) => {
  axios
    .get(`${process.env.REACT_APP_HEROKU_API_KEY}/users`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    })
...
export const blockedUsers = (dispatch, payload) => {
  axios
    .put(
      `${process.env.REACT_APP_HEROKU_API_KEY}/profile/${payload}`,
      {
        status: "blocked",
      },
      {
