import { Flex, Stack, Heading, Text, Input, Button, Icon, useColorModeValue, createIcon } from "@chakra-ui/react";
...
export default function NewsLetter() {
  ...
  return (
    <Flex align={"center"} justify={"center"} py={8} bg="gray.100">
      <Stack boxShadow={"2xl"} bg={boxBG} rounded={"xl"} p={6} spacing={3} align={"center"} w={"75%"}>
        <Stack align={"center"} spacing={2}>
          <Icon as={NotificationIcon} w={20} h={20} />
          <Heading textTransform={"uppercase"} fontSize={"2xl"} color={headingColor}>
            Subscribe
          </Heading>
        ...
        <Stack direction={{ base: "column", md: "row" }} w={"50%"}>
          <form
            ref={form}
            onSubmit={sendEmail}
            style={{ display: "flex", justifyContent: "center", padding: 0, margin: 0, width: "100%" }}
          >
            <Input
              name={"user_email"}
              type={"text"}
...
const NotificationIcon = createIcon({
  ...
  path: (
    <g id="Notification">
      ...
      />
      <path fill={"#f6b756"} className="cls-3" d="M109,107.63v4A22,22,0,0,1,83.09,88,22,22,0,0,0,109,107.63Z" />
      <path className="cls-5" fill={"#d6ac90"} d="M93,30l16,15L65.91,84.9a16,16,0,0,1-21.82,0L1,45,17,30Z" />
      <path
        className="cls-6"
        fill={"#cba07a"}
      ...
      />
      <path className="cls-3" fill={"#f6b756"} d="M74,53H36a21.36,21.36,0,0,0,1.86-4H72.14A21.36,21.36,0,0,0,74,53Z" />
